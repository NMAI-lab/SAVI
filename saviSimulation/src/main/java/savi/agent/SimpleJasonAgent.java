...
public class SimpleJasonAgent extends AgArch implements Runnable {
	...
	public SimpleJasonAgent(String id, String type, SyncAgentState modelAgentState) { //need to make the UAS class public so that the AgArch can refer back to it
		try {
			File logConfig = ResourceManager.getResource("logging.properties");
		...
		try {
			...
			this.name = id;
			File aslFile = ResourceManager.getResource("asl/" + type + ".asl");
            ag.initAg(aslFile.getCanonicalPath());
		} catch (Exception e) {
	...
	public void run(){
		logger.fine("I'm a Jason Agent and I'm starting");
	...
	public List<Literal> perceive() {
		...
		logger.fine("Agent " + getAgName() + " Perceiving perception "+ this.lastPerceptionId);
		logger.fine(perceptionLiterals.toString());
	...
	public void act(ActionExec action) {
		...
		logger.fine("MYAgent " + getAgName() + " is doing: " + actionTerm);
